...
/**
 * excludeBands: [{y0,y1,why}] in REFERENCE pixel coordinates — regions where the rebuild
 * deliberately deviates (a real form where the reference hides a modal, a skipped marquee).
 * Excluded rows are dropped from the mean and the worst-band scan and reported separately, so
 * the score reflects what the build was actually asked to match. Field incident: an agent spent
 * 8 minutes chasing a ≤8 target that was mathematically unreachable — the worst band (61.6) was
 * a brief-MANDATED deviation the gate had no way to know about.
 */
export async function run(env, { pageUrl, referencePath, viewportHeight = 900, keepShots = null, excludeBands = [] }) {
...
bands = json.loads(sys.argv[2])
def excluded(y):
    return any(b['y0'] <= y < b['y1'] for b in bands)
diffs = []
worst = {'band_y': None, 'mean': 0.0}
total = 0.0; rows = 0
ex_total = 0.0; ex_rows = 0
for s in slices:
    ...
    keep = np.array([not excluded(y0 + r) for r in range(h)])
    dk = d[keep]
    if dk.shape[0]:
        diffs.append({'y0': y0, 'h': h, 'mean': round(float(dk.mean()), 2)})
        total += float(dk.sum()); rows += dk.shape[0] * dk.shape[1]
    de = d[~keep]
    if de.shape[0]:
        ex_total += float(de.sum()); ex_rows += de.shape[0] * de.shape[1]
    for by in range(0, h - 49, 50):
        if any(excluded(y0 + by + i) for i in (0, 25, 49)): continue
        m = float(d[by:by+50].mean())
        if m > worst['mean']: worst = {'band_y': y0 + by, 'mean': round(m, 2)}
out = {'meanAbsDiff': round(total / max(rows, 1), 2), 'worstBand': worst, 'perSlice': diffs}
if ex_rows: out['excluded'] = {'bands': bands, 'meanAbsDiff': round(ex_total / ex_rows, 2)}
print(json.dumps(out))`;
    const sp = join(dir, '_score.py');
    writeFileSync(sp, script);
    const out = execFileSync('python3', [sp, JSON.stringify(slices), JSON.stringify(excludeBands)], { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
