...
export async function run(env, { pageId = null, heal = true } = {}) {
  ...
  const scans = [];
  for (const p of pages) { const sc = await scan(p); if (sc.hrefs.length) scans.push(sc); }
    ...
    for (const sc of scans) {
      ...
      push(`css:page-${sc.page.id}`, re.bad.length === 0,
        re.bad.length === 0
          ...
          : `${re.bad.length} stylesheet(s) STILL failing after the multi-page heal. Make the page store-independent: \`eu-studio carry-css --page ${sc.page.id}\` (or restart the WP worker and re-run doctor).`,
