...
export async function run(env, { pageId = null, heal = true } = {}) {
  ...
  for (const p of pages) {
    ...
    for (const href of hrefs) {
      const r = await fetchStatus(href.startsWith('http') ? href : env.url + href);
      if (!cssOk(r)) bad.push({ href, status: r.status });
    }
    if (!bad.length) { push(`css:page-${p.id}`, true, `${hrefs.length} stylesheet(s) serving`); continue; }

    if (!heal) { push(`css:page-${p.id}`, false, `${bad.length}/${hrefs.length} stylesheet(s) failing (no --no-heal)`); continue; }

    // heal: regen + prime with retries (prime is flaky on PHP-WASM; a fresh worker often succeeds
    // where a long-running one refuses — real incident 2026-08-07)
    // post-<id>.css failures (the KIT stylesheet is post-<kitId>.css) need a POST-SCOPED regen —
    // the global flush + page prime never rewrites them (real incident: post-4.css 404 survived
    // every heal until cache/regen {post_id:4}).
    for (const b of bad) {
      const m = /post-(\d+)\.css/.exec(b.href);
      if (m) await api(env, '/elementor-ultra/v1/cache/regen', { method: 'POST', body: JSON.stringify({ post_id: Number(m[1]) }) }).catch(() => {});
    }
    await api(env, '/elementor-ultra/v1/cache/regen', { method: 'POST' }).catch(() => {});
    let primed = false;
    for (let i = 0; i < 4 && !primed; i++) {
      const pr = await api(env, `/elementor-ultra/v1/documents/${p.id}/prime-css`, { method: 'POST', timeoutMs: 60000 }).catch(() => ({ ok: false }));
      primed = pr.ok && pr.body?.data?.css_primed;
      if (!primed) await sleep(2500);
    }
    if (!primed) await fetchStatus(`${env.url}/?page_id=${p.id}`); // a plain visit sometimes writes what prime won't

    let stillBad = [];
    for (const href of hrefs) {
      const r = await fetchStatus(href.startsWith('http') ? href : env.url + href);
      if (!cssOk(r)) stillBad.push(href);
    }
    push(`css:page-${p.id}`, stillBad.length === 0,
      stillBad.length === 0
        ? `healed — ${hrefs.length} stylesheet(s) serving after ${primed ? 'prime' : 'regen+visit'}`
        : `${stillBad.length} stylesheet(s) STILL failing after heal. Last resort: restart the WP server process (fresh PHP worker), or harvest CSS from a clean twin instance — see studio README "CSS harvest".`,
      true);
  }
