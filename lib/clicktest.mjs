 ...
 *   nav       click the nav link whose text OR href pathname matches the value after '=' → URL
 *             changes (href matching matters: a text-only matcher let agents bend the site's copy
 *             to satisfy the gate instead of fixing the nav — audited gate defect)
 *
 * JSON out; exit 1 on any failure. Runs each preset on its own page context.
 */
import { resolvePlaywright } from './env.mjs';

export async function run(env, { tests }, sharedBrowser = null) {
        ...
        if (t.kind === 'nav') {
          ...
          const clicked = await pg.evaluate((txt) => {
            ...
            const link = [...document.querySelectorAll('a[href]')].find((a) => {
              ...
              if (a.textContent.trim().toLowerCase().includes(t)) return true;
              try { return new URL(a.getAttribute('href'), location.href).pathname.toLowerCase().includes(t); } catch { return false; }
            });
